...
- 如果係 IG 帖文、廣告文案、或 Reels 腳本：輸出正文後，必須附上【🖼 配圖 Prompt（直接貼入 ChatGPT Image 2）】，英文一句話，格式：「[主體描述], [品牌風格], [光線], [構圖], photorealistic, 9:16 vertical」。A/B 兩版各配一個對應風格嘅 prompt。""",
...
【🔴 第一關：先決定係 clarify / reject / 執行，唔好猜】

▸ clarify（問清楚）— 指令有歧義，可以有多個解讀：
  - 冇具體目標（冇帳號、冇關鍵詞、冇明確成品）
  - 例：「幫我整個嘢」「分析下」「做份嘢」→ clarify
  - 格式：俾 A / B 具體選項，唔問開放問題

▸ reject（直接否決）— Amy 明白想做咩，但做唔到或冇資料：
  - 要分析/寫內容，但完全冇相關數據/資料喺對話入面
  - 要抓 IG（通常被封鎖）
  - 要做某成品，但缺少必要輸入（例如要翻譯但冇原文）
  - 系統根本冇呢個功能
  - reject 必須包含：① 做唔到原因（一句）② 建議點做、應該點講（具體指令例子）

▸ 執行 — 指令清晰 + 有足夠資料/能力 → 才派員工
...
def agent_call(agent_name: str, task: str) -> tuple[str, str]:
    ...
    user_msg = (
        f"Stanley 交俾你嘅任務：\n{task}\n\n"
        "【⚠️ 強制規則 — 所有員工必須遵守，按順序過三關】\n\n"
        "第一關：理解意圖\n"
        "Stanley 講嘢口語化、可能唔完整。你嘅責任係盡力從上下文推斷真實需求。\n"
        "- 有九成把握明白 → 直接執行，唔好問\n"
        "- 真係有關鍵細節唔清楚，做唔到 → 輸出：\n"
        "  ❓ 唔夠清楚，無法執行\n"
        "  我理解你想要：[你嘅最佳解讀]\n"
        ...
        "  你可以咁講：[正確指令例子]\n\n"
        "第二關：檢查真實數據\n"
        "真實數據 = 實際抓取結果 / 真實客戶原話 / 具體數字 / 真實競品資料\n"
        "如果任務需要真實數據但冇提供 → 唔好虛構，輸出：\n"
        "  ❌ 資料不足，無法執行\n"
        "  需要補充：[具體需要咩資料]\n"
        "  建議做法：[點樣獲取，例如 /scan [關鍵詞] 或 /scrape ig [帳號名]]\n\n"
        "第三關：執行\n"
        "兩關都過 → 提供最完整、最具體、可直接使用嘅成品，唔好省略任何重要細節。"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.get("reject"):
        reject_msg = f"{AGENT_EMOJI['Amy']} Amy：{data['reject']}"
        history.append({"role": "user", "content": user_message})
        history.append({"role": "assistant", "content": reject_msg})
        save_history(conversation_history)
        await send_long(update, reject_msg)
        return

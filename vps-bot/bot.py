...
def agent_call(agent_name: str, task: str) -> tuple[str, str]:
    ...
    user_msg = (
        f"Stanley 交俾你嘅任務：\n{task}\n\n"
        "【⚠️ 強制規則 — 所有員工必須遵守】\n"
        "執行任務前，先自問：「任務入面有冇足夠嘅真實數據？」\n"
        "真實數據 = 實際抓取結果 / 真實客戶原話 / 具體數字 / 真實競品資料\n"
        "如果任務入面冇呢啲，唔好自行填補、唔好虛構、唔好用假設數字。\n"
        "直接輸出以下格式，然後停止：\n"
        "❌ 資料不足，無法執行\n"
        "需要補充：[你需要嘅具體資料，例如：競品IG帖文數據 / 客戶真實反饋 / 市場調查數字]\n"
        "建議做法：[應該怎樣獲取，例如：先用 /scan [關鍵詞] 抓取數據，或者 /scrape ig [帳號名]]\n\n"
        "如果有足夠真實數據 → 提供最完整、最具體、可直接使用嘅成品，唔好省略任何重要細節。"
    )
